...
<!--
  👉 En cuanto vuelvas a desplegar en Render, pon aquí el enlace en vivo,
  justo debajo del título, antes que ningún otro texto. Es lo primero
  que mira un reclutador.
...
<!--
  👉 Añade aquí 2-3 capturas de pantalla: la web pública (hero + galería)
  y el panel de administración. Una imagen dice más que media página
  de texto, sobre todo si nadie va a clonar el repo para verlo.

  ![Web pública](docs/screenshot-home.png)
  ![Panel de administración](docs/screenshot-admin.png)
-->
...
Cada mes que termina se archiva solo: al crear una reserva nueva o al abrir el panel de admin, la aplicación comprueba si quedan citas de meses ya cerrados, cuenta cuántas hubo y las guarda en un histórico mensual, borrándolas después de la tabla activa. Así el panel se mantiene ligero sin perder el dato de cuántos clientes se atendieron cada mes, y sin depender de ninguna tarea programada (cron) externa que en un hosting gratuito no siempre está garantizado que se ejecute.
